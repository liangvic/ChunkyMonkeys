...
public class ChunkServerNode extends ServerNode {
	...
	public void DeleteChunk(ChunkMetadata metadata) {
		/*
		 * listMetaData chunkToDelete = null; boolean foundChunk = false;
		 * for(listMetaData lmd: files) { if(lmd.chunkHash ==
		 * metadata.chunkHash) { chunkToDelete = lmd; foundChunk = true; } }
		 * if(foundChunk) { files.remove(chunkToDelete); Message
		 * successMessageToMaster = new Message(msgType.DELETEDIRECTORY);
